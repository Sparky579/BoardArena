"""Gemini V3.4 (Alpha-Beta + Full Vulnerability).

Features:
- Alpha-Beta search.
- Full vulnerability calculation at leaf nodes (same as V2).
- Bitboard BFS for maximum speed.
"""
...
def get_intersecting_walls_set(path, limit=None):
    walls = set()
    limit = len(path) - 1 if limit is None else min(len(path) - 1, limit)
    for i in range(limit):
        u, v = path[i], path[i+1]
        if abs(u - v) == 9:
            top = min(u, v)
            r, c = top // 9, top % 9
            if c < 8: walls.add((0, r, c))
            if c > 0: walls.add((0, r, c - 1))
        elif abs(u - v) == 1:
            left = min(u, v)
            r, c = left // 9, left % 9
            if r < 8: walls.add((1, r, c))
            if r > 0: walls.add((1, r - 1, c))
    return walls
...
class Bot:
    ...
    def _alpha_beta(self, my_pos, opp_pos, my_goal, opp_goal, my_wrem, opp_wrem, h_mask, v_mask, h_walls, v_walls, depth, ply, alpha, beta):
        ...
        if depth == 0:
            md, my_path = bfs_path(my_pos, my_goal, h_mask, v_mask)
            if md >= INF: return -WIN_SCORE + ply, None
            od, opp_path = bfs_path(opp_pos, opp_goal, h_mask, v_mask)
            ...
            my_vuln = md
            if opp_wrem > 0:
                my_vuln = calc_vulnerability(my_pos, my_goal, my_path, h_mask, v_mask, h_walls, v_walls)
                
            opp_vuln = od
            if my_wrem > 0:
                opp_vuln = calc_vulnerability(opp_pos, opp_goal, opp_path, h_mask, v_mask, h_walls, v_walls)
                
            race_diff = opp_turns - my_turns
            race_diff -= (my_vuln - md) * 1.5
            race_diff += (opp_vuln - od) * 1.5
