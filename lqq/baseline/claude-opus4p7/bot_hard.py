"""Strong Quoridor-like bot. Bitboard + iterative-deepening alpha-beta + TT.

Time budget: 0.85 s per move (the ELO ladder uses a 1 s decision timeout, so
this leaves ~150 ms of headroom).

Components:
  - 81-bit board representation, parallel-BFS distance, sequential BFS path
  - move generation: 4 cardinal pawn moves + straight/diagonal jumps;
    focused wall set = walls touching the first ~12 edges of opp's shortest
    path + first 4 edges of own path
  - α-β with iterative deepening, PVS at root
  - transposition table keyed on (turn, p0, p1, w0, w1, H-wall mask, V-wall
    mask); mate-distance adjustment on store/probe
  - move ordering: TT move > pawn move along own shortest path > other pawn
    moves > walls scored by Δ(opp shortest-path distance)
  - mate-distance pruning
  - tapered evaluation: race difference (with tempo correction) + wall diff
    + center column bonus + tiny mobility hint
"""
...
def bfs_dist(start, goal_row, H, V):
    """Parallel-BFS shortest-path distance (returns INF if unreachable).

    H is the bitboard of blocked horizontal edges (bit i set = edge between
    cell i and cell i+9 is blocked). V analogous for vertical edges.
    """
...
def bfs_path(start, goal_row, H, V):
    """Sequential BFS, returns the list of cells on a shortest path."""
    parent = {start: -1}
    queue = deque([start])
    target_row_low = 0 if goal_row == 0 else 72
    target_row_high = 8 if goal_row == 0 else 80
    while queue:
        cell = queue.popleft()
        if target_row_low <= cell <= target_row_high and cell // 9 == goal_row:
            path = []
            while cell != -1:
                path.append(cell)
                cell = parent[cell]
...
def legal_pawn_dests(pos, opp, H, V):
    """Return list of destination cells the pawn at `pos` can step to.

    Implements straight moves (4 cardinals), straight jumps over an adjacent
    opponent, and diagonal side-jumps when a straight jump is blocked.
    """
    r, c = divmod(pos, 9)
    out = []
    seen = set()
    ...
    for dr, dc in ((-1, 0), (1, 0), (0, -1), (0, 1)):
        ...
        if not (0 <= nr <= 8 and 0 <= nc <= 8):
            continue
        # edge wall check between (r,c) and (nr,nc)
        if dr == -1:
            if H & (1 << (pos - 9)):
                continue
        elif dr == 1:
        ...
        # Adjacent to opponent. Try a straight jump.
        jump_done = False
        jr, jc = nr + dr, nc + dc
        ...
        if not jump_done:
            # Diagonal side jumps perpendicular to (dr, dc).
...
def walls_blocking_edge(a, b):
    """Walls that would block movement across the edge between cells a, b.

    Returns a list of (kind, wp) where kind=1 is H-wall, kind=2 is V-wall,
    and wp = wr*9 + wc is the wall origin.
    """
    ar, ac = divmod(a, 9)
    br, bc = divmod(b, 9)
    out = []
...
def wall_legal_quick(kind, wp, H, V, Hw, Vw):
    """Cheap legality check (in-bounds, no overlap or crossing or edge-coll).

    Does not check the path-existence invariant — caller must do that after
    applying the wall.
    """
    wr, wc = divmod(wp, 9)
...
def pawn_action_name(from_pos, to_pos):
    """Map (from, to) pawn step to its MOVE_* string."""
...
class Engine:
    def __init__(self):
        ...
        self.nodes = 0
        self.me = 0
        # Killer moves per ply (two slots, like a stack of recent cutoffs).
    ...
    def evaluate(turn, p0, p1, w0, w1, H, V):
        """Static eval in centi-units, from side-to-move's perspective.

        Higher = better for `turn`.
        """
        d0 = bfs_dist(p0, 0, H, V)
        d1 = bfs_dist(p1, 8, H, V)
        if d0 == INF or d1 == INF:
            return 0  # shouldn't happen with valid wall placements

        # Race score: an extra ply matters, so subtract a half-tempo from
        # whichever side moves next.
        ...
        # Mild central preference: each pawn likes being near col 4.
        c0 = p0 % 9
        c1 = p1 % 9
    ...
    @staticmethod
    def _focused_walls(turn, p0, p1, H, V, Hw, Vw, w_current,
                       opp_path, my_path):
        ...
        result = []
        for kind, wp in candidates:
            if not wall_legal_quick(kind, wp, H, V, Hw, Vw):
                continue
            new_H, new_V = apply_wall_edges_only(kind, wp, H, V)
            # Both players must still be able to reach their goal.
            if bfs_dist(p0, 0, new_H, new_V) >= INF:
                continue
            if bfs_dist(p1, 8, new_H, new_V) >= INF:
                continue
            result.append((kind, wp))
        return result

    # ---- move ordering ----

    def _score_move(self, m, tt_move, target_next, opp_dist_before,
                    p0, p1, H, V, opp_pos, opp_goal, killer1, killer2, depth):
        if m == tt_move:
            return 10**8
        if m[0] == 0:
            base = 2000
            ...
            # Forward-ish move
            return base
        # Wall: score by Δopp_distance (positive = slows opp).
        new_H, new_V = apply_wall_edges_only(m[0], m[1], H, V)
        new_dist = bfs_dist(opp_pos, opp_goal, new_H, new_V)
        delta = new_dist - opp_dist_before
        score = 800 + delta * 80
        if m == killer1:
            score += 120
        elif m == killer2:
            score += 60
        history_bonus = self.history.get(m, 0)
        return score + history_bonus
    ...
    def search(self, depth, alpha, beta, ply,
        ...
        # Terminal: if a goal row is occupied, the player who just moved won.
        if p0 < 9:
            return -(WIN_SCORE - ply)  # P0 just won; current `turn` is P1.
        if p1 >= 72:
        ...
        opp_dist_before = bfs_dist(opp_pos, opp_goal, H, V)
        ...
        moves.extend(self._focused_walls(
            turn, p0, p1, H, V, Hw, Vw, w_current, opp_path, my_path,
        ))

        if not moves:
            # No legal action available — current actor loses.
            return -(WIN_SCORE - ply)
        ...
        scored = [
            (
                self._score_move(
                    m, tt_move, target_next, opp_dist_before,
                    p0, p1, H, V, opp_pos, opp_goal, k1, k2, depth,
                ),
                i,
                m,
            )
        ...
        for _, _, m in scored:
            ...
            if score > best_value:
                ...
                if score > alpha:
                    alpha = score
                    if alpha >= beta:
                        # Killer & history bookkeeping for non-capture-equivalents
                        # — pawn moves and walls both qualify (no captures here).
                        if ply < MAX_PLY:
    ...
    def _search_root(self, depth, turn, p0, p1, w0, w1, H, V, Hw, Vw,
                     prev_best):
        pos = p0 if turn == 0 else p1
        opp_pos = p1 if turn == 0 else p0
        my_goal = 0 if turn == 0 else 8
        ...
        scored = [
            (
                self._score_move(
                    m, prev_best, target_next, opp_dist_before,
                    p0, p1, H, V, opp_pos, opp_goal, k1, k2, depth,
                ),
                i,
                m,
    ...
    def choose(self, state, budget=TIME_BUDGET):
        ...
        # Shortcut: if a 1-step move ends on our goal row, take it.
        pos = p0 if turn == 0 else p1
        my_goal = 0 if turn == 0 else 8
        ...
        for depth in range(1, 40):
            if depth > 2 and time.perf_counter() > self.deadline_soft:
                break
            try:
                _, m = self._search_root(
                    depth, turn, p0, p1, w0, w1, H, V, Hw, Vw, best_move,
                )
                if m is not None:
                    best_move = m
            except TimeUp:
    ...
    def _fallback(state, legal_actions, turn, p0, p1, H, V):
        # Walk along shortest path; otherwise return any move; otherwise any
        # legal action.
        pos = p0 if turn == 0 else p1
        my_goal = 0 if turn == 0 else 8
