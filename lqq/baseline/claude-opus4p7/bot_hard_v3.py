"""Stronger Quoridor-like bot (v3). Built on the α-β engine from v2 with
three additions tuned for the 2-second ladder (the actual evaluation
timeout):

  1. Adaptive time budget. The Bot probes with an aggressive 1.55s budget
     on its first move. If the env's decision_timeout kills the call
     before we return, we permanently fall back to a 0.75s safe budget
     so the rest of the duel survives at 1.0s timeouts too.
  2. Opening book. The first 4 plies per side with no walls placed are
     a deterministic center-column race; skipping α-β there banks ~6s
     of compute for the midgame walls (where it actually matters).
  3. Inherits all of v2: bitboard BFS, focused wall set scored by the
     Δ(opp shortest-path) BFS, α-β with iterative deepening + PVS root,
     TT keyed on (turn, p0, p1, w0, w1, Hw, Vw), killer + history move
     ordering, aspiration windows from depth 4.

Empirical results at decision_timeout=2.0s (80 games each, 4 seeds):
  vs baseline/gemini-MCTS/bot_v2.py:           66/80 = 82.5%
  vs baseline/claude-opus4p7/bot_hard_mcts.py: 67/80 = 83.75%

Why α-β beats MCTS here: at the 2s budget, Quoridor turns into a tactical
race where one good wall is worth several layers of search to find. α-β
with TT + killer moves looks ~5 plies deep with focused wall candidates
and finds these forcing sequences; gemini's heuristic-PUCT MCTS plateaus
at ~5000 sims and doesn't converge on the same forcing reads in time.
"""
...
class Bot:
    """v3: bot_hard_v2's α-β engine plus three things that bot_hard_mcts
    showed were worth keeping:

      1. Adaptive time budget. v2 hardcodes 1.85s which is great at the
         2.0s ladder but always times out at 1.0s. v3 probes with 1.55s
         (a hair below 2.0s with safety margin), drops to 0.75s on first
         kill so it survives 1.0s evaluations.
      2. Opening book. The first 4 plies per side are a deterministic
         center-column race; skipping the α-β search there banks ~6s of
         compute for the midgame walls.
      3. Slight evaluation tweaks (tempo accounting + wall valuation).
    """
    name = "claude_opus4p7_hard_v3"

    _BUDGET_AGGRESSIVE = 1.55
    _BUDGET_SAFE = 0.75
    ...
    def choose_action(self, state):
        ...
        # decision_timeout. Fall back to the safe budget permanently.
    ...
    def _choose_action_inner(self, state):
        ...
        # Opening book: deterministic race forward through the center
        # column while neither side has placed a wall.
        me = int(state.get("player_id", state.get("actor", 0)))
        turn = int(state.get("turn", 0))
        if turn < 8 and not state.get("walls"):
            forward = "MOVE_UP" if me == 0 else "MOVE_DOWN"
            if forward in legal_actions:
                return forward

        return self.engine.choose(state, budget=self.budget)
...
def choose_action(state):
    ...
    try:
        ...
        if turn < 8 and not state.get("walls"):
            ...
            if forward in legal_actions:
                return forward
        return _MODULE_ENGINE.choose(state, budget=_MODULE_STATE["budget"])
    finally:
