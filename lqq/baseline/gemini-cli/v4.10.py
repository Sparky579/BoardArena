"""Gemini V4.10 - The Aggressive Blocker.

Key features:
- High priority for walls that block opponent (score > pawn path).
- Lower wall weight in evaluation to encourage usage.
- Optimized search and BFS.
- Persistent TT and Aspiration Windows.
"""
...
class Engine:
    ...
    def evaluate(self, p0, p1, w0, w1, hm, vm, turn):
        ...
        # Low wall weight to encourage usage
        score = (r1 - r0) * 100 + (w0 - w1) * 5
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
        ...
        if my_w > 0:
            cands = set(); od_now = len(opp_path) - 1
            ...
            for d, r, c in cands:
                ...
                # HIGH priority for walls if they block opponent at root
                # For search nodes, use a high default but less than path moves
                if depth >= 4:
                    i9 = r * 9 + c
                    if d == 0: nh, nv = hm | (1 << i9) | (1 << (i9 + 1)), vm
                    else: nh, nv = hm, vm | (1 << i9) | (1 << (idx9 := i9 + 9))
                    ndo = bfs_dist(opp_p, opp_g, nh, nv)
                    if ndo >= 255: continue
                    # Score can be very high if it blocks opponent!
                    moves.append((100 + d*64 + r*8 + c, 700000 + (ndo - od_now) * 50000))
                else:
                    moves.append((100 + d*64 + r*8 + c, 600000))
        ...
        for i, (sc, a) in enumerate(scored):
            ...
            if a < 8:
                ...
                if bfs_dist(n_my, my_g, n_hm, n_vm) >= 255: continue
