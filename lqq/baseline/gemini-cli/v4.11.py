"""Gemini V4.11 - The Persistent Challenger.

Key features:
- Matches V3 evaluation weights (Race weight 24, Wall weight 2).
- Persistent TT (unlike V3's clear-per-turn).
- Aspiration Windows and PVS.
- Optimized wall generation (matching V3's logic).
"""
...
class Engine:
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
        ...
        if my_w > 0:
            cands = set()
            for path, lim in [(opp_path, 10), (my_path, 2)]:
            ...
            for d, r, c in cands:
                ...
                # Basic wall score
                moves.append((100 + d*64 + r*8 + c, 400000))
        ...
        for i, (sc, a) in enumerate(scored):
            ...
            if a < 8:
                ...
                if bfs_dist(n_my, my_g, n_hm, n_vm) >= 255 or bfs_dist(opp_p, opp_g, n_hm, n_vm) >= 255: continue
            
            n_p0, n_p1, n_w0, n_w1 = (n_my, opp_p, n_wrem, opp_w) if turn == 0 else (opp_p, n_my, opp_w, n_wrem)
            
            # PVS
            if i == 0: sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-1, ply+1, -beta, -alpha)
            else:
