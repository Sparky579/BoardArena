"""Gemini V4.13 - The Mirror of V3.

Key features:
- Matches V3 engine exactly (no path checks in loop, high penalty for blocked states).
- Persistent TT, Zobrist Hashing, Aspiration Windows, PVS.
- Fast evaluation and BFS.
"""
    ...
    while front:
        ...
        if front & target_mask:
            ...
            for d in range(dist - 1, -1, -1):
                ...
                elif r > 0 and (prev & (1 << (curr - 11))) and not (h_mask & (1 << (curr - 9))): # Wait, V3's backtrack logic is better
                    pass
                # I'll use a safer backtrack
                if r < 8 and (prev & (1 << (curr + 9))) and not (h_mask & (1 << curr)): curr += 9
...
class Engine:
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
                ...
                if not b:
                    if m9 != opp_p: raw.append(act)
                        ...
                        if 0 <= jr <= 8 and 0 <= jc <= 8:
                            ...
                            if not jb: raw.append(act); jd = True
                        if not jd:
        ...
        for a in raw:
            ...
            moves.append((sc, a))
        if my_w > 0:
            owalls = get_path_walls(opp_path, 10); mwalls = get_path_walls(my_path, 4); seen = set()
            for d, wr, wc in owalls:
                i8 = wr*8+wc
                if d==0:
                    if hw&(1<<i8) or (wc>0 and hw&(1<<(i8-1))) or (wc<7 and hw&(1<<(i8+1))) or vw&(1<<i8): continue
                else:
                    if vw&(1<<i8) or (wr>0 and vw&(1<<(i8-8))) or (wr<7 and vw&(1<<(i8+8))) or hw&(1<<i8): continue
        ...
        for i, (sc, a) in enumerate(scored):
            ...
            if a < 8:
                ...
                v = a-100; d, rem = divmod(v, 64); r, c = divmod(rem, 8); i8, i9, nw = r*8+c, r*9+c, nw-1
                if d == 0: nhw |= (1 << i8); nhm |= (1 << i9) | (1 << (i9 + 1))
                else: nvw |= (1 << i8); nvm |= (1 << i9) | (1 << (i9 + 9))
