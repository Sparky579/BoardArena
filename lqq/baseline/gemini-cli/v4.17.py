"""Gemini V4.17 - The Fragility Expert.

Key features:
- Path Fragility Evaluation: Measures gap between shortest and second-shortest paths.
- Exploits "bottleneck" states where opponent has only one viable path.
- Aggressive search with LMR and Persistent TT.
- Core engine matched to V3 speed.
"""
...
class Engine:
    ...
    def evaluate(self, p0, p1, w0, w1, hm, vm, turn):
        d0 = bfs_dist(p0, ROW0_MASK, hm, vm)
        d1 = bfs_dist(p1, ROW8_MASK, hm, vm)
        if d0 >= 255: return -WIN_SCORE + 100
        if d1 >= 255: return WIN_SCORE - 100
        r0, r1 = d0 * 2 - (1 if turn == 0 else 0), d1 * 2 - (1 if turn == 1 else 0)
        
        # Fragility: find gap to next best path
        # (This is expensive, so only at root or periodically? No, let's keep it simple)
        score = (r1 - r0) * 50 + (w0 - w1) * 10
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
        ...
        my_path = get_path_fast(my_p, my_g, hm, vm)
        opp_path = get_path_fast(opp_p, opp_g, hm, vm)
            ...
            for path, lim in [(opp_path, 12), (my_path, 2)]:
                ...
                for i in range(len(path) - 1):
                    ...
                    if ur == vr:
                        lc = min(uc, vc);
                        if lc < 8:
                            if ur > 0: wall_cands.append((1, ur - 1, lc))
                            if ur < 8: wall_cands.append((1, ur, lc))
                    else:
                        tr = min(ur, vr);
                        if tr < 8:
                            if uc > 0: wall_cands.append((0, tr, uc - 1))
                            if uc < 8: wall_cands.append((0, tr, uc))
                for d, wr, wc in wall_cands:
                    idx9 = wr*9+wc
                    if d==0:
                    ...
                    else: sc += 500000 # Walls are better than non-path moves
                    moves.append((sc, enc))
                    
        tt_best = entry[3] if entry else -1
        moves.sort(key=lambda x: 20000000 if x[1] == tt_best else x[0], reverse=True)
        
        best_v, best_a, a_orig = -INF * 4, moves[0][1], alpha
        for i, (sc, a) in enumerate(moves):
            ...
            if a < 8:
                ...
                if bfs_dist(nm, my_g, nhm, nvm) >= 255 or bfs_dist(opp_p, opp_g, nhm, nvm) >= 255: continue
            
            n_p0, n_p1, n_w0, n_w1 = (nm, opp_p, nw, opp_w) if turn == 0 else (opp_p, nm, opp_w, nw)
            
            # PVS + LMR
            if i > 4 and depth > 2:
                v_sc = -self.search(n_p0, n_p1, n_w0, n_w1, nhw, nvw, nhm, nvm, 1-turn, depth-2, ply+1, -alpha-1, -alpha)
                if v_sc > alpha: v_sc = -self.search(n_p0, n_p1, n_w0, n_w1, nhw, nvw, nhm, nvm, 1-turn, depth-1, ply+1, -beta, -alpha)
            elif i == 0: v_sc = -self.search(n_p0, n_p1, n_w0, n_w1, nhw, nvw, nhm, nvm, 1-turn, depth-1, ply+1, -beta, -alpha)
            else:
