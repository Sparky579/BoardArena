"""Gemini V4.9 - The Deep Runner.

Key features:
- Search nodes matched to V3 speed (no BFS wall scoring in search).
- BFS wall scoring ONLY at the root to pick candidates.
...
class Engine:
    def __init__(self):
        ...
        self.stop_time, self.max_tt_size, self.nodes = 0, 600000, 0
                        ...
                        if not jd:
                            for pr, pc, pact in [(0,-1,4),(0,1,5)] if dr != 0 else [(-1,0,4),(1,0,6)]:
                                ...
                                if 0 <= sr <= 8 and 0 <= sc <= 8:
                                    ...
                                    if not sb:
                                        moves.append((pact if dr != 0 else (4 if pr == -1 and dc == -1 else (5 if pr == -1 and dc == 1 else (6 if pr == 1 and dc == -1 else 7))), 800000))
        if my_w > 0:
            cands = set()
