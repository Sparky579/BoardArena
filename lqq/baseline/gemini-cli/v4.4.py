"""Gemini V4.4 - The Symmetrical Sentinel.

Key features:
- Horizontal Symmetry in TT: Normalizes states to exploit board symmetry.
- Increased wall candidate pool (top 14 of opp path).
- Refined evaluation with better tempo/wall balance.
- Internal Iterative Deepening (IID) for better move ordering.
- Persistent TT, Zobrist Hashing, Aspiration Windows, PVS, LMR.
"""
...
def get_horiz_mirror_state(p0, p1, hw, vw):
    # pos = r*9 + c -> r*9 + (8-c)
    m_p0 = (p0 // 9) * 9 + (8 - (p0 % 9))
    m_p1 = (p1 // 9) * 9 + (8 - (p1 % 9))
    # wall = r*8 + c -> r*8 + (7-c)
    ...
    for i in range(64):
        if hw & (1 << i):
            r, c = divmod(i, 8); m_hw |= (1 << (r * 8 + (7 - c)))
        if vw & (1 << i):
            r, c = divmod(i, 8); m_vw |= (1 << (r * 8 + (7 - c)))
    return m_p0, m_p1, m_hw, m_vw
...
def get_normalized_hash(p0, p1, w0, w1, hw, vw, turn):
    ...
    mp0, mp1, mhw, mvw = get_horiz_mirror_state(p0, p1, hw, vw)
...
def get_dist(pos, target_row, h_mask, v_mask):
    d, _ = get_dist_and_redundancy(pos, target_row, h_mask, v_mask); return d
...
class Engine:
    ...
    def evaluate(self, p0, p1, w0, w1, h_mask, v_mask, turn):
        ...
        score = (race1 - race0) * 100 + (w0 - w1) * 110 # Slightly higher wall weight
        score += (r0 - r1) * 2
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
        ...
        if my_wrem > 0:
            ...
            for d, r, c in cands:
                ...
                if ndm >= 255: continue
                moves.append((100 + d*64 + r*8 + c, 500000 + (ndo - od_now) * 20000 - (ndm - md_now) * 15000))
        ...
        for i, (s, a) in enumerate(scored):
            ...
            if i > 5 and depth > 2:
                sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-2, ply+1, -alpha-1, -alpha)
