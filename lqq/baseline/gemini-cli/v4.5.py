"""Gemini V4.5 - The Deep Strategist.

Key features:
- Aggressive LMR: Searches much deeper on PV.
- Conservative Wall Evaluation: Avoids premature wall usage.
- State-aware move ordering.
- Horizontal Symmetry in TT.
- Highly optimized bitboard operations.
"""
...
def get_normalized_hash(p0, p1, w0, w1, hw, vw, turn):
    h1 = get_state_hash(p0, p1, w0, w1, hw, vw, turn)
    # Horizontal mirror: c -> 8-c
    mp0 = (p0 // 9) * 9 + (8 - (p0 % 9))
    mp1 = (p1 // 9) * 9 + (8 - (p1 % 9))
    mhw, mvw = 0, 0
    for i in range(64):
        if hw & (1 << i): r, c = divmod(i, 8); mhw |= (1 << (r * 8 + (7 - c)))
        if vw & (1 << i): r, c = divmod(i, 8); mvw |= (1 << (r * 8 + (7 - c)))
    h2 = get_state_hash(mp0, mp1, w0, w1, mhw, mvw, turn)
    return min(h1, h2)
...
class Engine:
    ...
    def evaluate(self, p0, p1, w0, w1, hm, vm, turn):
        ...
        # Tuned weights: Tempi is priority, walls are auxiliary
        score = (race1 - race0) * 100 + (w0 - w1) * 30
        score += (4 - abs(p0 % 9 - 4)) * 5 - (4 - abs(p1 % 9 - 4)) * 5
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
        ...
        h = get_normalized_hash(p0, p1, w0, w1, hw, vw, turn)
        entry = self.tt.get(h)
        ...
        my_path, opp_path = get_path(my_pos, my_goal, hm, vm), get_path(opp_pos, opp_goal, hm, vm)
        ...
        for dr, dc, act in [(-1,0,0),(1,0,1),(0,-1,2),(0,1,3)]:
            ...
            if 0 <= nr <= 8 and 0 <= nc <= 8:
                m9 = nr*9 + nc; b = False
                if dr == -1: b = hm & (1 << (my_pos - 9))
                elif dr == 1: b = hm & (1 << my_pos)
                elif dc == -1: b = vm & (1 << (my_pos - 1))
                else: b = vm & (1 << my_pos)
                if not b:
                    if m9 != opp_pos: moves.append((act, 800000 if m9 == my_path[1] else 100000))
                        ...
                        if 0 <= jr <= 8 and 0 <= jc <= 8:
                            jb = False
                            if dr == -1: jb = hm & (1 << (opp_pos - 9))
                            elif dr == 1: jb = hm & (1 << opp_pos)
                            elif dc == -1: jb = vm & (1 << (opp_pos - 1))
                            else: jb = vm & (1 << opp_pos)
                        ...
                        if not jd:
                            for pr, pc, pact in [(0,-1,4),(0,1,5)] if dr != 0 else [(-1,0,4),(1,0,6)]:
                                ...
                                if 0 <= sr <= 8 and 0 <= sc <= 8:
                                    sb = False
                                    if pr == -1: sb = hm & (1 << (opp_pos - 9))
                                    elif pr == 1: sb = hm & (1 << opp_pos)
                                    elif pc == -1: sb = vm & (1 << (opp_pos - 1))
                                    else: sb = vm & (1 << opp_pos)
                                    if not sb:
        ...
        if my_wrem > 0:
            ...
            for path, lim in [(opp_path, 14), (my_path, 2)]:
            ...
            for d, r, c in cands:
                ...
                ndo = get_dist(opp_pos, opp_goal, nh, nv)
                if ndo >= 255: continue
                ndm = get_dist(my_pos, my_goal, nh, nv)
                if ndm >= 255: continue
                moves.append((100 + d*64 + r*8 + c, 500000 + (ndo - od_now) * 30000 - (ndm - md_now) * 10000))
        ...
        for i, (s, a) in enumerate(scored):
            n_my, n_wrem, n_hw, n_vw, n_hm, n_vm = my_pos, my_wrem, hw, vw, hm, vm
            if a < 8:
                ...
                elif a == 4: n_my = opp_pos-1 if n_my-9==opp_pos or n_my-1==opp_pos and orow<r else opp_pos-9
                elif a == 5: n_my = opp_pos+1 if n_my-9==opp_pos or n_my+1==opp_pos and orow<r else opp_pos-9
                elif a == 6: n_my = opp_pos-1 if n_my+9==opp_pos or n_my-1==opp_pos and orow>r else opp_pos+9
                elif a == 7: n_my = opp_pos+1 if n_my+9==opp_pos or n_my+1==opp_pos and orow>r else opp_pos+9
            else:
            ...
            # More aggressive LMR
            if i > 3 and depth > 2:
                red = 2 if i > 12 else 1
                sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-1-red, ply+1, -alpha-1, -alpha)
                if sc > alpha: sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-1, ply+1, -beta, -alpha)
            elif i == 0: sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-1, ply+1, -beta, -alpha)
            else:
                sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-1, ply+1, -alpha-1, -alpha)
                if alpha < sc < beta: sc = -self.search(n_p0, n_p1, n_w0, n_w1, n_hw, n_vw, n_hm, n_vm, 1-turn, depth-1, ply+1, -beta, -alpha)
            
            if sc > best_v: best_v, best_a = sc, a
            if sc > alpha:
                alpha = sc
                if alpha >= beta:
    ...
    def choose(self, state):
        ...
        for w in state.get("walls", []):
            ...
            if w["dir"] == "H": hw |= (1 << i8); hm |= (1 << i9) | (1 << (idx9 := i9 + 1))
            else: vw |= (1 << i8); vm |= (1 << i9) | (1 << (idx9 := i9 + 9))
        ...
        try:
            for depth in range(1, 40):
                if depth > 3:
                    ...
                    while True:
                        sc = self.search(p0, p1, w0, w1, hw, vw, hm, vm, me, depth, 0, alpha, beta)
                        if sc <= alpha: alpha -= 100
                        elif sc >= beta: beta += 100
                        else: break
                    last_score = sc
                else: last_score = self.search(p0, p1, w0, w1, hw, vw, hm, vm, me, depth, 0, -WIN_SCORE*4, WIN_SCORE*4)
                h = get_normalized_hash(p0, p1, w0, w1, hw, vw, me); best_a = self.tt[h][3]
