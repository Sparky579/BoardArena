"""Gemini V4.8 - The Hybrid Titan.

Key features:
- Inherits V3's hyper-fast bitboard engine.
- Persistent TT with Zobrist Hashing (unlike V3's clear-per-turn).
- Aspiration Windows & Principal Variation Search (PVS).
- Late Move Reductions (LMR) for extreme depth.
- Refined evaluation (Tempi, Walls, Bottlenecks).
- Horizontal Symmetry for TT efficiency.
"""
...
class Engine:
    def __init__(self):
        ...
        self.stop_time, self.max_tt_size = 0, 600000
    ...
    def search(self, p0, p1, w0, w1, hw, vw, hm, vm, turn, depth, ply, alpha, beta):
        ...
        if my_w > 0:
            ...
            for d, r, c in cands:
                ...
                if depth >= 4:
                    ndo = bfs_dist(opp_p, opp_g_mask, nh, nv)
                    if ndo >= 255: continue
                    moves.append((100 + d*64 + r*8 + c, 500000 + (ndo - od_now) * 10000))
                else:
        ...
        for i, (sc, a) in enumerate(scored):
            ...
            if a < 8:
                ...
                if bfs_dist(n_my, my_g_mask, n_hm, n_vm) >= 255: continue
