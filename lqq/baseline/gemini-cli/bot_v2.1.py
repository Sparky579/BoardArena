...
Features:
- Bitwise BFS for 2500+ MCTS nodes / second.
- PATH reconstruction fixed to perfectly match V2's "hug the wall" behavior.
- V2's exact mathematical race bug and priors retained.
- Added Dirichlet noise at the root to prevent tunnel vision.
"""
    ...
    while front:
        if front & target_mask:
            ...
            for d in range(dist - 1, -1, -1):
                ...
                # Match V2 behavior: V2 checks UP, DOWN, LEFT, RIGHT in forward traversal.
                # So to replicate, backwards traversal must prioritize LEFT/RIGHT first.
...
def get_legal_actions_with_priors(state, my_dist, my_path, opp_dist, opp_path):
    ...
    for move, dest in moves:
        if dest // 9 == state.goals[me]: return [(move, 1000.0)]
    ...
    if state.walls_rem[me] > 0:
        ...
        for d, wr, wc in tactical:
            if is_valid_wall(d, wr, wc, state.h_walls, state.v_walls):
                ...
                # V2 Priors exactly
                if (d, wr, wc) in opp_path_walls:
                    if od > opp_dist: actions.append((f"WALL_{d}_{wr}_{wc}", 8.0))
...
class MCTS:
    ...
    def search(self, state):
        ...
        # Dirichlet Noise at root to encourage broader exploration
        if self.root.children:
            noise = [random.gammavariate(0.3, 1.0) for _ in self.root.children]
            noise_sum = sum(noise)
            if noise_sum > 0:
                for i, c in enumerate(self.root.children):
                    c.P = 0.75 * c.P + 0.25 * (noise[i] / noise_sum)
        ...
        while True:
            ...
            while node.is_expanded and node.children:
                ...
                for child in node.children:
                    if child.N == 0: q_node = parent_q
                    else: q_node = -(child.W / child.N)
            ...
            if not node.is_expanded: value = self.expand_and_evaluate(node)
            else:
                me = node.state.actor
                if node.state.pos[me] // 9 == node.state.goals[me]: value = 1.0
                else: value = -1.0
