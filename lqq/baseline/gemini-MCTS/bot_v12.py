...
- Exact math for turn-based racing.
- "Conservative Margin" mathematically parameterized based on V2's successful bug.
- Smart Trap Priors.
"""
    ...
    if state.walls_rem[me] > 0:
        ...
        for d, wr, wc in tactical:
            if is_valid_wall(d, wr, wc, state.h_walls, state.v_walls):
                ...
                # Reward walls that severely delay the opponent while not delaying us too much
                my_delay = md - my_dist
                opp_delay = od - opp_dist
                
                if opp_delay > 0:
                    if opp_delay >= 2 and my_delay <= 0: actions.append((f"WALL_{d}_{wr}_{wc}", 80.0))
                    elif opp_delay > my_delay: actions.append((f"WALL_{d}_{wr}_{wc}", 20.0))
                    else: actions.append((f"WALL_{d}_{wr}_{wc}", 5.0))
                else:
...
class MCTS:
    ...
    def expand_and_evaluate(self, node):
        ...
        # Exact turn requirement
        my_turns = my_dist * 2 - 1
        opp_turns = opp_dist * 2
        
        race_diff = opp_turns - my_turns 
        
        # PARAMETERIZED CONSERVATISM: 
        # Replicating V2's exact mathematical offset that made it naturally trap-setting and risk-averse.
        race_diff -= 2.0 
