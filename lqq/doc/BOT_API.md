...
- `MOVE_UP_LEFT` / `MOVE_UP_RIGHT`：当对手在正上方相邻格时，分别跳到对手左侧或右侧一格；目标格必须在棋盘内，且对手到目标格之间不能有墙。
- `MOVE_DOWN_LEFT` / `MOVE_DOWN_RIGHT`：当对手在正下方相邻格时，分别跳到对手左侧或右侧一格；目标格必须在棋盘内，且对手到目标格之间不能有墙。
- `MOVE_UP_LEFT` / `MOVE_DOWN_LEFT`：当对手在左侧相邻格时，分别跳到对手上方或下方一格；目标格必须在棋盘内，且对手到目标格之间不能有墙。
- `MOVE_UP_RIGHT` / `MOVE_DOWN_RIGHT`：当对手在右侧相邻格时，分别跳到对手上方或下方一格；目标格必须在棋盘内，且对手到目标格之间不能有墙。
- `WALL_H_r_c`：在原点 `(r, c)` 放置横墙。
- `WALL_V_r_c`：在原点 `(r, c)` 放置竖墙。

移动动作的字符串表示最终移动方向；当该方向触发跳跃规则时，裁判会把棋子移动到对手后方一格，或移动到对手侧边一格。
