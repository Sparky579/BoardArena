"""Gemini V3.1 (Ultra-Fast Alpha-Beta).

Features:
- Lazy Legality: BFS is only used AFTER applying a wall deep in the tree.
- Static Wall Generation: Walls are generated by merely intersecting existing shortest paths.
...
def get_path_walls_scored(path, base_score, max_edges):
    walls = {}
    for i in range(min(len(path) - 1, max_edges)):
        ...
        if abs(u - v) == 9:
            top = min(u, v)
            r, c = top // 9, top % 9
            if c < 8: walls[(0, r, c)] = max(walls.get((0, r, c), 0), score)
            if c > 0: walls[(0, r, c - 1)] = max(walls.get((0, r, c - 1), 0), score)
        elif abs(u - v) == 1:
            left = min(u, v)
            r, c = left // 9, left % 9
            if r < 8: walls[(1, r, c)] = max(walls.get((1, r, c), 0), score)
            if r > 0: walls[(1, r - 1, c)] = max(walls.get((1, r - 1, c), 0), score)
    return walls
...
class Bot:
    ...
    def _generate_moves(self, my_pos, opp_pos, my_path, opp_path, my_wrem, h_mask, v_mask, h_walls, v_walls):
        actions = []
        ...
        for a in actions:
            ...
            if dest == next_pos: score = 5000
            scored_actions.append((score, a))
            
        if my_wrem > 0:
            opp_walls = get_path_walls_scored(opp_path, 4000, 12)
            my_walls = get_path_walls_scored(my_path, 2000, 4)
            
            tactical = {}
            for w, s in my_walls.items(): tactical[w] = s
            for w, s in opp_walls.items(): tactical[w] = max(tactical.get(w, 0), s)
            
            for (d, wr, wc), score in tactical.items():
    ...
    def _alpha_beta(self, my_pos, opp_pos, my_goal, opp_goal, my_wrem, opp_wrem, h_mask, v_mask, h_walls, v_walls, depth, alpha, beta):
        self.nodes += 1
        if self.nodes % 2048 == 0 and time.perf_counter() > self.stop_time:
            raise TimeoutError()
            
        if my_pos // 9 == my_goal: return WIN_SCORE + depth, None
        if opp_pos // 9 == opp_goal: return -WIN_SCORE - depth, None
        ...
        if depth == 0:
            ...
            if md >= INF: return -WIN_SCORE, None
            od = bfs_dist_only(opp_pos, opp_goal, h_mask, v_mask)
            if od >= INF: return WIN_SCORE, None
        ...
        if my_dist >= INF: return -WIN_SCORE, None
        if opp_dist >= INF: return WIN_SCORE, None
            
        actions = self._generate_moves(my_pos, opp_pos, my_path, opp_path, my_wrem, h_mask, v_mask, h_walls, v_walls)
        if not actions: return -WIN_SCORE, None
