...
def get_shortest_path_dist(start_pos, goal_row, H_edges, V_edges):
    ...
    while front:
        if front & target_mask:
            return dist
        can_up = front & ~(H_edges << 9)
        can_down = front & ~H_edges
        can_left = front & NOT_LEFT_COL_MASK & ~(V_edges << 1)
        can_right = front & NOT_RIGHT_COL_MASK & ~V_edges
        
        new_up = can_up >> 9
        new_down = can_down << 9
        new_left = can_left >> 1
        new_right = can_right << 1
        
        front = ((new_up | new_down | new_left | new_right) & BOARD_MASK) & ~visited
        visited |= front
        dist += 1
    return 1000
...
def get_legal_moves(pos, opp_pos, H_edges, V_edges):
    moves = []
    def add_side_jumps(from_pos, vertical):
        if vertical:
            if (from_pos % 9) > 0 and not (V_edges & (1 << (from_pos - 1))):
                moves.append(from_pos - 1)
            if (from_pos % 9) < 8 and not (V_edges & (1 << from_pos)):
                moves.append(from_pos + 1)
        else:
            if from_pos >= 9 and not (H_edges & (1 << (from_pos - 9))):
                moves.append(from_pos - 9)
            if from_pos < 72 and not (H_edges & (1 << from_pos)):
                moves.append(from_pos + 9)
...
class Bot:
    ...
    def generate_focused_walls(self, me, p0, p1, H_edges, V_edges, H_walls, V_walls):
        ...
        goal = 8 if opp == 1 else 0
        opp_path = get_shortest_path(opp_pos, goal, H_edges, V_edges)
    ...
    def search(self, depth, alpha, beta, turn, p0, p1, w0, w1, H_edges, V_edges, H_walls, V_walls):
        ...
        state_hash = (turn) | (p0 << 1) | (p1 << 8) | (w0 << 15) | (w1 << 19) | (H_walls << 23) | (V_walls << 87)
    ...
    def move_to_str(self, m, p0, p1, turn):
        if m[0] == 0:
            ...
            opp_pos = p1 if turn == 0 else p0
            npos = m[1]
            if npos == pos - 9: return "MOVE_UP"
            if npos == pos + 9: return "MOVE_DOWN"
            if npos == pos - 1: return "MOVE_LEFT"
            if npos == pos + 1: return "MOVE_RIGHT"
            if npos == pos - 18 and opp_pos == pos - 9: return "MOVE_UP"
            if npos == pos + 18 and opp_pos == pos + 9: return "MOVE_DOWN"
            if npos == pos - 2 and opp_pos == pos - 1: return "MOVE_LEFT"
            if npos == pos + 2 and opp_pos == pos + 1: return "MOVE_RIGHT"
            if npos == pos - 10: return "MOVE_UP_LEFT"
            if npos == pos - 8: return "MOVE_UP_RIGHT"
            if npos == pos + 8: return "MOVE_DOWN_LEFT"
            if npos == pos + 10: return "MOVE_DOWN_RIGHT"
            return "MOVE_UP" # Fallback
        elif m[0] == 1:
            r = m[1] // 9
            c = m[1] % 9
    ...
    def choose_action(self, state):
        ...
        path = get_shortest_path(my_pos, my_goal, H_edges, V_edges)
        if len(path) > 1:
            action_str = self.move_to_str((0, path[1]), p0, p1, turn)
            if action_str in legal:
                return action_str
