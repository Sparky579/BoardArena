"""Round-robin tournament for chess baseline bots.
100 games per matchup, 3s decision timeout, ELO calculation.

Usage:
    python chess/run_tourney.py [--games 100] [--timeout 3.0] [--output tourney_results.json]
"""
...
from env.chess_env import ChessEnv, load_bot, DEFAULT_MAX_PLIES
...
def play_one_game(bot_white, bot_black, seed: int, decision_timeout: float, max_plies: int):
...
def main():
    parser = argparse.ArgumentParser(description="Chess baseline bot round-robin tournament")
