"""Strong chess bot. Pure traditional search, no neural networks.

Thinking budget: ~1.6 s per move (configurable; the engine aborts the current
iteration once 85 % of the budget has elapsed).

Components:
  - iterative deepening with PVS (principal variation search)
  - quiescence search with delta pruning
  - transposition table keyed on a compact position tuple
  - move ordering: TT move > promotions > MVV-LVA captures > killers > history
  - null-move pruning and late-move reductions
  - check extension
  - PeSTO-style tapered evaluation (material + piece-square tables) plus
    bishop-pair, doubled/isolated pawn, and rook-on-(semi-)open-file terms
"""
...
class Engine:
    ...
    def _search_root(self, board, depth, prev_best):
        ...
        fall back to the previous iteration's best move.
        """
        legal = list(board.generate_legal_moves())
        if not legal:
            return 0, None
