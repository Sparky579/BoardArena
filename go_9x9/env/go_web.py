#!/usr/bin/env python3
"""Local browser interface for BoardArena 9x9 Go."""
...
from go_env import DEFAULT_KOMI, DEFAULT_MAX_PLIES, PASS_ACTION, BotTimeoutError, Go9x9Env, SystemBot, choose_action_with_timeout, load_bot
...
class GameSession:
    def __init__(
        ...
        *,
        ...
        self.env = Go9x9Env(seed=seed, max_plies=max_plies, komi=komi)
    ...
    def _push_action(self, action: str, *, source: str) -> None:
        ...
        if action == PASS_ACTION:
            ...
            suffix = f"，提 {len(self.env.last_captures)} 子" if self.env.last_captures else ""
            text = f"{who}: {action}{suffix}"
        ...
        if terminated or truncated:
            ...
            if winner is None:
                text = f"结束: {status}，黑 {scores[0]:.1f} : 白 {scores[1]:.1f}"
                self.log.append({"ply": self.env.plies, "seat": -1, "action": "", "text": text})
            else:
                text = f"结束: {SIDE_LABELS[winner]}胜，黑 {scores[0]:.1f} : 白 {scores[1]:.1f}"
                self.log.append({"ply": self.env.plies, "seat": winner, "action": "", "text": text})


class GoServer(ThreadingHTTPServer):
    def __init__(
        self,
...
def bind_server(args: argparse.Namespace) -> tuple[GoServer, int]:
