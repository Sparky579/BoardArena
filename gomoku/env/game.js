...
function showRequestError(error, options = {}) {
  ...
  if (isMissingApi) {
    statusText.textContent = "请用 python BoardArena/gomoku/env/gomoku_web.py 启动；Live Server 没有本地 JSON API。";
  } else if (error.isNetworkError) {
    statusText.textContent = "连接本地 gomoku_web.py 服务失败，请确认服务仍在运行。";
  } else {
...
function render() {
  ...
  blackScore.textContent = String(data.scores[0]);
  whiteScore.textContent = String(data.scores[1]);
