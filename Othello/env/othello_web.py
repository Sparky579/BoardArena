#!/usr/bin/env python3
"""Local browser interface for BoardArena Othello."""
...
from othello_env import DEFAULT_MAX_PLIES, PASS_ACTION, BotTimeoutError, OthelloEnv, SystemBot, choose_action_with_timeout, load_bot
...
def bot_id_for_path(path: Path) -> str:
    """Stable ``/<provider>/<bot_stem>`` id used by the UI dropdown."""
    try:
...
def discover_bot_paths() -> dict[str, Path]:
    ...
    deterministic ``{bot_id: path}`` mapping. New bots are picked up
    just by dropping the file in the right baseline subdirectory; no
    manual registration in othello_web.py needed.
    """
...
class GameSession:
    def __init__(
        ...
        *,
        ...
        self.env = OthelloEnv(seed=seed, max_plies=max_plies)
    ...
    def status_text(self, state: dict[str, Any]) -> str:
        ...
        if state["phase"] == "game_over":
            winner = state["winner"]
            if winner is None:
                return f"平局，黑 {scores[0]} : 白 {scores[1]}"
            return f"{SIDE_LABELS[winner]}获胜，黑 {scores[0]} : 白 {scores[1]}"

        side = SIDE_LABELS[state["actor"]]
        if state["legal_actions"] == [PASS_ACTION]:
            return f"{side}无合法落子，必须 Pass"
    ...
    def _push_action(self, action: str, *, source: str) -> None:
        ...
        if action == PASS_ACTION:
            ...
            text = f"{who}: {action}，翻 {len(self.env.last_flips)} 子"
...
class OthelloServer(ThreadingHTTPServer):
    def __init__(
        self,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the BoardArena Othello browser UI")
    ...
    print(f"BoardArena Othello UI: {url}")
...
def bind_server(args: argparse.Namespace) -> tuple[OthelloServer, int]:
