    ...
    if state.walls_rem[me] > 0:
        ...
        for d, wr, wc in tactical:
            if is_valid_wall(d, wr, wc, state.h_walls, state.v_walls):
                ...
                if (d, wr, wc) in opp_path_walls:
                    if od > opp_dist + 4: actions.append((f"WALL_{d}_{wr}_{wc}", 200.0))
                    elif od > opp_dist + 2: actions.append((f"WALL_{d}_{wr}_{wc}", 80.0))
                    elif od > opp_dist: actions.append((f"WALL_{d}_{wr}_{wc}", 15.0))
                    else: actions.append((f"WALL_{d}_{wr}_{wc}", 1.5))
                else:
                    actions.append((f"WALL_{d}_{wr}_{wc}", 2.5)) # Strong prior to explore traps
...
class MCTS:
    ...
    def expand_and_evaluate(self, node):
        ...
        race_diff -= (my_vuln - my_dist) * 3.0 # Extra heavy penalty for being trapped
        race_diff += (opp_vuln - opp_dist) * 3.0
        
        value = math.tanh(race_diff * 0.3)
        value += (state.walls_rem[me] - state.walls_rem[opp]) * 0.05
...
class Bot:
    ...
    def choose_action(self, state_dict):
        ...
        # Opening Book
        if turn < 2:
            if me == 0 and "MOVE_DOWN" in legal_actions: return "MOVE_DOWN"
            if me == 1 and "MOVE_UP" in legal_actions: return "MOVE_UP"
