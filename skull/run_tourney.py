"""Round-robin tournament for Skull baseline bots.
...
from env.skull_env import load_bot, run_match
...
def derive_pairwise(bot_names: list[str], scores_list: list[list[int]]) -> dict:
    """Derive pairwise wins/losses/draws from game scores.
    Higher score = better in Skull (win count).
...
def main():
    parser = argparse.ArgumentParser(description="Skull baseline bot tournament")
    ...
    for g in range(args.games):
        ...
        try:
            ...
            scores_list.append(list(result.scores))
